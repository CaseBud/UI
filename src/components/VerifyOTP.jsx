...
const VerifyOTP = () => {
  ...
  const handleSubmit = async (e, autoOtp = null) => {
    ...
    try {
      const response = await fetch('https://case-bud-backend-bzgqfka6daeracaj.centralus-01.azurewebsites.net/api/auth/verify-email', {
        method: 'POST',
        headers: {
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="mt-auto">
            ...
            <p className="mt-4 text-lg text-blue-100">
              We're committed to protecting your account with secure verification.
            </p>
            <div className="mt-8 grid grid-cols-2 gap-4">
              <div className="bg-blue-900/20 backdrop-blur-sm p-4 rounded-lg">
                ...
                <p className="text-blue-100 text-sm mt-1">Two-factor authentication</p>
              </div>
              <div className="bg-blue-900/20 backdrop-blur-sm p-4 rounded-lg">
                <h3 className="text-white font-semibold">Quick & Easy</h3>
                <p className="text-blue-100 text-sm mt-1">Verify in seconds</p>
              </div>
            </div>
          </div>
        </div>
                  ...
                  {[...Array(6)].map((_, i) => (
                    <div
                      ...
                      className={`w-3 h-1 rounded-full transition-all duration-200 ${
                        i < otp.length ? 'bg-blue-500' : 'bg-slate-600'
                      }`}
                    />
                  ))}
                </div>
